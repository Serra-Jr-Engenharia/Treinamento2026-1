          ...
          >
            <div className="flex gap-5 p-6">
              ...
              <div className="shrink-0 w-36 rounded-xl overflow-hidden bg-gray-200">
                {selected.Poster && selected.Poster !== 'N/A' ? (
                  ...
                  <div className="w-full h-48 flex items-center justify-center text-gray-400 text-sm">Sem Poster</div>
                )}
              </div>
              ...
              <div className="flex flex-col gap-2 flex-1">
                ...
                <div className="flex flex-wrap gap-2 text-sm">
                  <span className="bg-orange-500 text-white px-3 py-0.5 rounded-full font-medium">{selected.Year}</span>
                  {selected.Runtime && selected.Runtime !== 'N/A' && (
                    <span className="bg-gray-100 text-gray-700 px-3 py-0.5 rounded-full">{selected.Runtime}</span>
                  )}
                  {selected.imdbRating && selected.imdbRating !== 'N/A' && (
                    <span className="bg-yellow-400 text-yellow-900 px-3 py-0.5 rounded-full font-medium">★ {selected.imdbRating}</span>
                  )}
                </div>
                ...
                {selected.Director && selected.Director !== 'N/A' && (
                  <p className="text-sm text-gray-600"><span className="font-medium">Diretor:</span> {selected.Director}</p>
                )}
                {selected.Actors && selected.Actors !== 'N/A' && (
                  <p className="text-sm text-gray-600"><span className="font-medium">Elenco:</span> {selected.Actors}</p>
                )}
              </div>
            </div>
            ...
            {selected.Plot && selected.Plot !== 'N/A' && (
              <div className="px-6 pb-4">
                <h3 className="text-sm font-bold text-gray-700 mb-1 uppercase tracking-wide">Sinopse</h3>
                <p className="text-sm text-gray-600 leading-relaxed">{selected.Plot}</p>
              </div>
            )}
